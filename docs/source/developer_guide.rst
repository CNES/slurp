...
.. code-block:: bash
    ...
    ## Update to real URL !
    git clone https://github.com/CNES/slum
    cd slum
    make install
...
* **Factorization**: Factorize the code as much as possible. The command line tools shall only include the main workflow and rely on the **slum** python modules.
...
slum includes a set of tests executed with `pytest <https://docs.pytest.org/>`_ tool.
...
It launches the unit tests present in `slum/tests/test_slum.py` displaying the traces generated by the tests and the tests code coverage level.

During the tests execution, slum will write the output data in a */tmp* directory.

Documentation
*************

slum documentation can be generated with following command:
...
This installs the pre-commit hook in `.git/hooks/pre-commit` and `.git/hooks/pre-push` from `.pre-commit-config.yaml <https://raw.githubusercontent.com/CNES/slum/master/.pre-commit-config.yaml>`_ file configuration.
...
.. code-block:: console
  ...
  $ pre-commit run --files slum/__init__.py   # Run all hooks on one file
...
``isort`` configuration is done in `.pyproject.toml <https://raw.githubusercontent.com/CNES/slum/master/pyproject.toml>`_
`Isort`_ manual usage examples:

.. code-block:: console

    $ cd slum_HOME
    $ isort --check slum tests  # Check code with isort, does nothing
    $ isort --diff slum tests   # Show isort diff modifications
    $ isort slum tests          # Apply modifications
...
``black`` configuration is done in `.pyproject.toml <https://raw.githubusercontent.com/CNES/slum/master/pyproject.toml>`_
...
.. code-block:: console
    ...
    $ cd slum_HOME
    $ black --check slum tests  # Check code with black with no modifications
    $ black --diff slum tests   # Show black diff modifications
    $ black slum tests          # Apply modifications

Flake8
------
`Flake8`_ is a command-line utility for enforcing style consistency across Python projects. By default it includes lint checks provided by the PyFlakes project, PEP-0008 inspired style checks provided by the PyCodeStyle project, and McCabe complexity checking provided by the McCabe project. It will also run third-party extensions if they are found and installed.

``flake8`` configuration is done in `setup.cfg <https://raw.githubusercontent.com/CNES/slum/master/setup.cfg>`_
...
.. code-block:: console
  ...
  $ cd slum_HOME
  $ flake8 slum tests           # Run all flake8 tests
...
``pylint`` configuration is done in dedicated `.pylintrc <https://raw.githubusercontent.com/CNES/slum/master/.pylintrc>`_ file.
...
.. code-block:: console
  ...
  $ cd slum_HOME
  $ pylint tests slum       # Run all pylint tests
...
Any proven or suspected malfunction should be traced in a bug report, the latter being an issue in the **slum** github repository.
...
**slum**'s Classical workflow is :
 - Check Licence and sign :ref:`contribution_license_agreement` (Individual or Corporate)
 - Create an issue (or begin from an existing one)
 - Create a Merge Request from the issue: a MR is created accordingly with *WIP:*, *Closes xx* and associated *xx-name-issue* branch
 - Modify **slum**'s code from a local working directory or from the forge (less possibilities)
...
**slum** requires that contributors sign out a `Contributor LicenseAgreement <https://en.wikipedia.org/wiki/Contributor_License_Agreement>`_.
...
`Individual Contributor LicensingAgreement <https://github.com/CNES/slum/blob/master/docs/source/CLA/ICLA-slum.doc>`_ (ICLA) form and a
`Corporate Contributor Licensing Agreement <https://github.com/CNES/slum/blob/master/docs/source/CLA/CCLA-slum.doc>`_ (CCLA) form if you are
...
modified source files and also added to the `AUTHORS.md file <https://github.com/CNES/slum/blob/master/AUTHORS.md>`_.
