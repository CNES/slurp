...
The arguments of `main_config` can be completed (and/or overwritten) by a second JSON file provided to the SLURP's API or CLI in the `user_config` argument.
Below is an example of a user configuration file:

.. include:: user_config_descr.md
   :parser: myst_parser.sphinx_

Finally, some arguments defined by the v file can be overwritten by command line arguments or by API's arguments.
