...
#    if np.count_nonzero(mask_building) < 2000:
 #       print('=> Warning : low water pixel number in Pekel Mask\n')
    ...
    # Building samples
    rows_b, cols_b = get_indexes_from_masks(nb_building_samples, im_gt, args.value_classif, valid_stack)

    rows_nob = []
    cols_nob = []
    if args.road_mask:
        ds_roads = rio.open(args.road_mask)
        im_roads = ds_roads.read(1)
        nb_samples = nb_other_samples / 2
        rows_nob0, cols_nob0 = get_indexes_from_masks(nb_samples, im_roads, 1, valid_stack)
        ...
        rows_nob, cols_nob = get_indexes_from_masks(nb_other_samples, im_gt, 0, valid_stack)

    save_indexes("samples.tif", zip(rows_b, cols_b), zip(rows_nob, cols_nob),
                 args.shape, args.crs, args.transform, args.rpc)
...
#               crs, transform, 255, rpc)
    ...
    # Show output images
    if args.display:
        show_images(im_predict, 'Predict image', mask_building, 'Mask building', vmin=0, vmax=1)
...
def getarguments():
    ...
    parser.add_argument('file_phr',
                        help='PHR filename')

    parser.add_argument('-red', default=1,
                        help="Red band index")
    ...
    parser.add_argument('-ndvi', default=None, required=False,
                        action='store', dest='file_ndvi',
                        help='NDVI filename (computed if missing option)')

    parser.add_argument('-ndwi', default=None, required=False,
                        action='store', dest='file_ndwi',
                        help='NDWI filename (computed if missing option)')

    parser.add_argument('-use_rgb_layers', default=False, required=False,
                        action='store_true', dest='use_rgb_layers',
                        help='Add R,G,B layers to image stack for learning')

    parser.add_argument('-layers', nargs='+', default=[], required=False,
                        action='store', dest='files_layers', metavar='FILE_LAYER',
                        help='Add layers as features used by learning algorithm')

    parser.add_argument('-building_mask', default=None, required=True,
                        action='store', dest='raster_building')

    parser.add_argument('-road_mask', default=None, required=False,
                        action='store', dest='road_mask', help='Provide a road layer to better separate roads from buildings')

    parser.add_argument('file_classif',
                        help='Output classification filename (default is classif.tif)')

    parser.add_argument('-value_classif', type=int, default=1, required=False,
