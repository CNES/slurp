...
from skimage.morphology import (area_closing, binary_closing, diameter_closing,
                                remove_small_holes, square)
...
def save_image(image, file, crs=None, transform=None, nodata=None, rpc=None,
               colormap=None, tags=None, **kwargs):
...
def superimpose(file_in, file_ref, file_out, type_out):
    ...
    app.SetParameterString('elev.geoid', "/softs/projets/cars/data/geoides/egm96.grd")
...
def pekel_recovery(file_ref, file_out):
    ...
    superimpose('/work/datalake/static_aux/MASQUES/PEKEL/data2021/occurrence/occurrence.vrt',
                file_ref, file_out, otb.ImagePixelType_uint8)
...
def pekel_month_recovery(file_ref, month, file_data_out, file_mask_out):
    ...
    superimpose('/work/datalake/static_aux/MASQUES/PEKEL/data2021/MonthlyRecurrence/'
                f'monthlyRecurrence{month}/monthlyRecurrence{month}.vrt',
                file_ref, file_data_out, otb.ImagePixelType_int16)

    superimpose('/work/datalake/static_aux/MASQUES/PEKEL/data2021/MonthlyRecurrence/'
                f'has_observations{month}/has_observations{month}.vrt',
                file_ref, file_mask_out, otb.ImagePixelType_int16)
...
def hand_recovery(file_ref, file_out):
    ...
    superimpose('/work/datalake/static_aux/MASQUES/HAND_MERIT/'
                'hnd.vrt', file_ref, file_out, otb.ImagePixelType_float)
...
def cloud_from_gml(file_cloud, file_ref):
    ...
    mask_cloud = app.GetImageAsNumpyArray('out', otb.ImagePixelType_uint8).astype(np.uint8)
...
def compute_ndxi(im_b1, im_b2, valid):
    ...
    start_time = time.time()
    im_ndxi = 1000. - (2000. * np.float32(im_b2)) / (np.float32(im_b1) + np.float32(im_b2))
    im_ndxi[np.logical_or(im_ndxi < -1000., im_ndxi > 1000.)] = np.nan
...
def get_smart_indexes_from_mask(nb_indexes, pct_area, minimum, mask):
    ...
    n1_indexes = int((1. - pct_area / 100.) * nb_indexes / nb_labels)

    # number of samples to distribute to each label/prop
    n2_indexes = pct_area / 100. * nb_indexes / mask_area
...
def show_rftree(estimator, feature_names):
    ...
    print(export_text(estimator, show_weights=True,
                      feature_names=feature_names))
...
def build_stack(args):
    ...
    save_image(valid_phr.astype(np.uint8), join(dirname(args.file_classif), 'valid.tif'),
               ds_phr.crs, ds_phr.transform, nodata=None, rpc=ds_phr.tags(ns='RPC'))
    ...
    if args.file_ndvi:
        ...
        im_ndvi, valid_ndvi = compute_ndxi(im_phr[names_stack.index('NIR')],
                                           im_phr[names_stack.index('R')],
                                           valid_phr)
        save_image(im_ndvi, join(dirname(args.file_classif), 'ndvi.tif'),
                   ds_phr.crs, ds_phr.transform, nodata=32767, rpc=ds_phr.tags(ns='RPC'))
    print('NDVI : min =', np.min(im_ndvi, where=(im_ndvi != 32767), initial=1000),
          'max =', np.max(im_ndvi, where=(im_ndvi != 32767), initial=-1000))
    ...
    if args.file_ndwi:
        ...
        im_ndwi, valid_ndwi = compute_ndxi(im_phr[names_stack.index('G')],
                                           im_phr[names_stack.index('NIR')],
                                           valid_phr)
        save_image(im_ndwi, join(dirname(args.file_classif), 'ndwi.tif'),
                   ds_phr.crs, ds_phr.transform, nodata=32767, rpc=ds_phr.tags(ns='RPC'))
    print('NDWI : min =', np.min(im_ndwi, where=(im_ndwi != 32767), initial=1000),
          'max =', np.max(im_ndwi, where=(im_ndwi != 32767), initial=-1000))
    ...
    if args.use_rgb_layers:
        im_stack = np.stack((*im_phr, im_ndvi, im_ndwi,
                             *(rio.open(file_layer).read(1) for file_layer in args.files_layers)))
    else:
        names_stack = ['NIR']
        im_stack = np.stack((im_phr[names_stack.index('NIR')], im_ndvi, im_ndwi,
                             *(rio.open(file_layer).read(1) for file_layer in args.files_layers)))
...
def build_samples(im_stack, valid_stack, args):
    ...
    if args.file_pekel is None:
        if 1 <= args.pekel_month <= 12:
            args.file_data_pekel = join(dirname(args.file_classif), f'pekel{args.pekel_month}.tif')
            args.file_mask_pekel = join(dirname(args.file_classif), f'has_observations{args.pekel_month}.tif')
            args.file_pekel = args.file_data_pekel
            pekel_month_recovery(args.file_phr, args.pekel_month, args.file_data_pekel, args.file_mask_pekel)
        else:
    ...
    if args.hand_strict:
        mask_pekelxx = compute_mask(args.file_pekel, args.strict_thresh, 'PEKEL')[0]
    ...
    if args.display:
        show_images(mask_pekel, 'PEKEL > ' + str(args.thresh_pekel),
                    mask_hand, 'HAND <' + str(args.thresh_hand))
    ...
    if args.samples_method != 'grid':
        ...
        if args.nb_samples_auto:
            ...
            nb_valid_water_pixels = np.count_nonzero(np.logical_and(mask_pekel, valid_stack))
        ...
        if args.samples_method == 'random':
            rows_pekel, cols_pekel = get_random_indexes_from_masks(nb_water_samples, valid_stack, mask_pekel)

        if args.samples_method == 'smart':
            rows_pekel, cols_pekel = get_smart_indexes_from_mask(nb_water_samples, args.smart_area_pct,
                                                                 args.smart_minimum,
                                                                 np.logical_and(mask_pekel, valid_stack))

        # Hand samples, always random (currently)
        rows_hand, cols_hand = get_random_indexes_from_masks(nb_other_samples, valid_stack, mask_hand)
        ...
        colormap = {0: (0, 0, 0, 0),  # nodata
                    ...
                    2: (255, 0, 0),  # autre
                    3: (0, 0, 0, 0)}
        save_indexes("samples.tif", zip(rows_pekel, cols_pekel), zip(rows_hand, cols_hand),
                     args.shape, args.crs, args.transform, args.rpc, colormap)

    else:
        # Prepare regular samples
        rows, cols = get_grid_indexes_from_masks(valid_stack, args.grid_spacing)

        rc_pekel = [(row, col) for (row, col) in zip(rows, cols) if mask_pekel[row, col] == 1]
        rc_others = [(row, col) for (row, col) in zip(rows, cols) if mask_pekel[row, col] != 1]

        colormap = {0: (0, 0, 0, 0),  # nodata
                    1: (0, 0, 255),  # eau
                    2: (255, 0, 0),  # autre
                    3: (0, 0, 0, 0)}
        save_indexes("samples.tif", rc_pekel, rc_others,
                     args.shape, args.crs, args.transform, args.rpc, colormap)
...
def train_classifier(classifier, x_samples, y_samples):
    ...
    print('Accuracy on train set :', accuracy_score(y_train, classifier.predict(x_train)))
...
def classify(args):
    ...
    if args.file_cloud_gml:
        mask_nocloud = np.logical_not(cloud_from_gml(args.file_cloud_gml, args.file_phr))
    else:
        mask_nocloud = np.ones(valid_stack.shape, dtype=np.uint8)

    if args.display:
        show_images(mask_nocloud, 'No Cloud image', valid_stack, 'Validity image', vmin=0, vmax=1)

    # Build samples from stack and control layers (pekel, hand)
    valid_samples = np.logical_and(valid_stack, mask_nocloud)
    x_samples, y_samples, mask_pekel, mask_hand = build_samples(im_stack, valid_samples, args)

    # Create and train classifier from samples
    classifier = RandomForestClassifier(n_estimators=100, max_depth=3, random_state=0, n_jobs=4)
    ...
    if args.pekel_filter:
        ...
        if args.file_esri:
            ...
            im_classif = mask_filter(im_predict, np.logical_or(mask_pekel0, mask_esri))
        else:
    ...
    if args.binary_closing:
        ...
        im_classif = diameter_closing(im_classif, args.diameter_closing, connectivity=2)
    elif args.area_closing:
        im_classif = area_closing(im_classif, args.area_closing, connectivity=2)
    elif args.remove_small_holes:
        im_classif = remove_small_holes(im_classif.astype(bool), args.remove_small_holes, connectivity=2).astype(np.uint8)
    ...
    save_image(im_predict, join(dirname(args.file_classif), 'predict.tif'),
               crs, transform, 255, rpc, tags=args.__dict__)
    save_image(im_classif, args.file_classif,
               crs, transform, 255, rpc, tags=args.__dict__)

    # Show output images
    if args.display:
        show_images(im_predict, 'Predict image', im_classif, 'Classif image', vmin=0, vmax=1)
...
def getarguments():
    ...
    group3 = parser.add_argument_group(description='*** LEARNING SAMPLES SELECTION ***')
    group4 = parser.add_argument_group(description='*** POST PROCESSING ***')
    group5 = parser.add_argument_group(description='*** OUTPUT FILE ***')

    # Input files
    group1.add_argument('file_phr',
                        help='PHR filename')

    group1.add_argument('-pekel', default=None, required=False,
                        action='store', dest='file_pekel',
                        help='Pekel filename (computed if missing option)')

    group1.add_argument('-thresh_pekel', type=float, default=50, required=False,
                        action='store', dest='thresh_pekel',
                        help='Pekel Threshold float (default is 50)')

    group1.add_argument('-pekel_month', type=int, default=0, required=False,
                        action='store', dest='pekel_month',
                        help='Use monthly recurrence map instead of occurence map')

    group1.add_argument('-hand', default=None, required=False,
                        action='store', dest='file_hand',
                        help='Hand filename (computed if missing option)')

    group1.add_argument('-thresh_hand', type=int, default=25, required=False,
                        action='store', dest='thresh_hand',
                        help='Hand Threshold int >= 0 (default is 25)')

    group1.add_argument('-ndvi', default=None, required=False,
                        action='store', dest='file_ndvi',
                        help='NDVI filename (computed if missing option)')

    group1.add_argument('-ndwi', default=None, required=False,
                        action='store', dest='file_ndwi',
                        help='NDWI filename (computed if missing option)')

    group1.add_argument('-layers', nargs='+', default=[], required=False,
                        action='store', dest='files_layers', metavar='FILE_LAYER',
                        help='Add layers as features used by learning algorithm')

    group1.add_argument('-cloud_gml', required=False,
                        action='store', dest='file_cloud_gml',
                        help='Cloud file in .GML format')

    group1.add_argument('-esri', default=None, required=False,
                        action='store', dest='file_esri',
                        help='ESRI filename, will be used in postprocessing')

    # Options
    group2.add_argument('-red', default=1, 
                        help="Red band index")

    group2.add_argument('-use_rgb_layers', default=False, required=False,
                        action='store_true', dest='use_rgb_layers',
                        help='Add R,G,B layers to image stack for learning')

    group2.add_argument('-hand_strict', default=False, required=False,
                        action='store_true', dest='hand_strict',
                        help='Use not(pekelxx) for other (no water) samples')

    group2.add_argument('-strict_thresh', type=float, default=50, required=False,
                        action='store', dest='strict_thresh',
                        help='Pekel Threshold float (default is 50)')

    group2.add_argument('-display', required=False,
                        action='store_true',
                        help='Display images while running')

    # Samples
    group3.add_argument('-samples_method', choices=['smart', 'grid', 'random'], default='smart', required=False,
                        action='store', dest='samples_method',
                        help='Select method for choosing learning samples')

    group3.add_argument('-nb_samples_water', type=int, default=2000, required=False,
                        action='store', dest='nb_samples_water',
                        help='Number of samples in water for learning (default is 2000)')

    group3.add_argument('-nb_samples_other', type=int, default=10000, required=False,
                        action='store', dest='nb_samples_other',
                        help='Number of samples in other for learning (default is 10000)')

    group3.add_argument('-nb_samples_auto', default=False, required=False,
                        action='store_true', dest='nb_samples_auto',
                        help='Auto select number of samples for water and other')

    group3.add_argument('-auto_pct', type=float, default=0.0002, required=False,
                        action='store', dest='auto_pct',
                        help='Percentage of samples points, to use with -nb_samples_auto')

    group3.add_argument('-smart_area_pct', type=int, default=50, required=False,
                        action='store', dest='smart_area_pct',
                        help='For smart method, importance of area for selecting number of samples in each water surface.')

    group3.add_argument('-smart_minimum', type=int, default=10, required=False,
                        action='store', dest='smart_minimum',
                        help='For smart method, minimum number of samples in each water surface.')

    group3.add_argument('-grid_spacing', type=int, default=40, required=False,
                        action='store', dest='grid_spacing',
                        help='For grid method, select samples on a regular grid (40 pixels seems to be a good value)')

    # Post processing
    group4.add_argument('-pekel_filter', default=True, required=False,
                        action='store_true', dest='pekel_filter',
                        help='Postprocess with pekel, only keep surfaces already known by pekel')

    group4.add_argument('-hand_filter', default=False, required=False,
                        action='store_true', dest='hand_filter',
                        help='Postprocess with Hand (set to 0 when hand > thresh), incompatible with hand_strict')

    group4.add_argument('-binary_closing', type=int, required=False,
                        action='store', dest='binary_closing',
                        help='Size of square structuring element')

    group4.add_argument('-diameter_closing', type=int, required=False,
                        action='store', dest='diameter_closing',
                        help='The maximal extension parameter (number of pixels)')

    group4.add_argument('-area_closing', type=int, required=False,
                        action='store', dest='area_closing',
                        help='Area closing removes all dark structures')

    group4.add_argument('-remove_small_holes', type=int, required=False,
                        action='store', dest='remove_small_holes',
                        help='The maximum area, in pixels, of a contiguous hole that will be filled')

    # Output
    group5.add_argument('file_classif',
                        help='Output classification filename')

    group5.add_argument('-value_classif', type=int, default=1, required=False,
                        action='store', dest='value_classif',
