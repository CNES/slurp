    ...
    with eom.EOContextManager(
        ...
        try:
            ...
            if args.analyse_glcm:
                nb_clusters_veg, nb_clusters_low_veg = (
            ...
            if args.overwrite or not path.isfile(args.valid_stack):
                ...
                eoscale_manager.write(
                    key=key_valid_stack[0], img_path=args.valid_stack
                )
            else:
                print(
            ...
            if args.texture_rad:
                if args.overwrite or not path.isfile(args.file_texture):
                    ...
                    percentiles = np.percentile(
                        eoscale_manager.get_array(key_phr)[3], [2, 98]
                    )
                    params = {
                        "nir": args.nir,
                    ...
                    print(
                        "Not computing texture file : the file already exists."
                    )
            else:
                ...
                for key in final_used_config:
                    for sub_key in final_used_config[key]:
                        if sub_key in argsdict:
                            final_used_config[key].update(
                                {sub_key: argsdict[sub_key]}
                            )

            makedirs(path.dirname(args.effective_used_config), exist_ok=True)
            with open(
                args.effective_used_config, "w", encoding="utf8"
            ) as file_to_save:
