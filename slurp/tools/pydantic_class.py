...
class Prepare(BaseModel):
    ...
    pekel: str = Field(..., description="Path to the Pekel occurrence data")
    pekel_monthly_occurrence: str = Field(..., description="Path to the monthly occurrence data for Pekel")
    pekel_obs: Optional[str] = Field(None, description="Path to Pekel observation data")
    hand: str = Field(..., description="Path to the Height Above the Nearest Drainage (HAND) model")
    wsf: str = Field(..., description="Path to the World Settlement Footprint (WSF) data")
...
class Urban(BaseModel):
    files_layers: List[str] = Field(..., description="List of file paths to urban layers")
    vegmask_min_value: int = Field(..., description="Minimum value for vegetation mask in urban analysis")
...
class Masks(BaseModel):
    watermask: str = Field(..., description="Path to the water mask file")
    urbanmask: str = Field(..., description="Path to the urban mask file")
    vegetationmask: str = Field(..., description="Path to the vegetation mask file")
    shadowmask: str = Field(..., description="Path to the shadow mask file")
    stackmask: str = Field(..., description="Path to the stack mask file")

class AuxLayersMain(BaseModel):
    valid_stack: str = Field(..., description="Path to the valid stack layer")
    file_ndvi: str = Field(..., description="Path to the NDVI layer")
    file_ndwi: str = Field(..., description="Path to the NDWI layer")
    extracted_pekel: str = Field(..., description="Path to the extracted Pekel data")
    extracted_hand: str = Field(..., description="Path to the extracted HAND data")
    extracted_wsf: str = Field(..., description="Path to the extracted WSF data")
    file_texture: str = Field(..., description="Path to the texture layer")
