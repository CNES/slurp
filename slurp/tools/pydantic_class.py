...
class StackMain(BaseModel):
    ...
    categorized_watermask: bool = Field(..., description="If true, stack_mask will infer water body category (lake, river, sea, unknown) from a general water body mask")
    minimal_size_water_area: int = Field(..., description="Minimal area (in pixels) of water bodies")
...
class AuxLayersMain(BaseModel):
    ...
    extracted_wbm: Optional[str] = Field(..., description="Path to the extracted Water Body Mask")
