...
def generate_polygons_in_gdf(im, crs, transform):
    ...
    gdf = gpd.GeoDataFrame(
        {"id": list(range(0, len(geometry))), "geometry": geometry}, crs=crs
    )
    return gdf
...
def get_union_gdf(gdf, gdf_ref, gdf_predict, crs):
    ...
    for index in indexes:
        ...
        gdf_unary = gdf[
            (gdf["id_1"] == index) | (gdf["id_2"].isin(index_predict.values))
        ]["geometry"]
        gdf_unary = pd.concat([gdf_unary, gdf_ref.iloc[[index]]["geometry"]])
        gdf_unary = pd.concat(
            [gdf_unary, gdf_predict.iloc[index_predict.values]["geometry"]]
        )
        polys.append(gdf_unary.unary_union)
    gdf_union = gpd.GeoDataFrame(
        {"id_1": indexes.astype("int"), "geometry": polys}, crs=crs
    )
    return gdf_union
...
def buildings_count(
    ...
    gdf_predict = generate_polygons_in_gdf(
        im_predict, crs_predict, transform_predict
    )
    ...
    if args.union:
        ...
        df_merged = gdf_intersect_area.merge(
            gdf_ref_filtered.geometry.area.reset_index(name="area_ref"),
            left_on="id_1",
            right_on="index",
        ).drop(columns="index")
        df_merged = df_merged.merge(
            gdf_union[["id_1", "area_union"]], left_on="id_1", right_on="id_1"
        )
...
def get_score(im_ref, im_predict):
    ...
    logger.info("Accuracy >>> %.2f", accuracy_score(im_ref, im_predict))
    ...
    logger.info("Jaccard >>> %.2f", jaccard_score(im_ref, im_predict))
...
def getarguments():
    ...
    parser.add_argument(
        "-im", required=True, action="store", help="Prediction image"
    )
    parser.add_argument(
        "-out", required=True, action="store", help="Output filename"
    )
    parser.add_argument(
        "-vcr",
        required=False,
        action="store",
        dest="value_classif_ref",
        type=int,
        nargs="+",
        default=[1],
