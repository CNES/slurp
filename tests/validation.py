...
def compare_datasets(ds_new_mask, ds_ref_mask):
    ...
    assert ds_new_mask.colorinterp == ds_ref_mask.colorinterp, \
        get_assert_message(ds_new_mask.colorinterp, ds_ref_mask.colorinterp, "colorinterp")
    assert ds_new_mask.tag_namespaces() == ds_ref_mask.tag_namespaces(), \
        get_assert_message(ds_new_mask.tag_namespaces(), ds_ref_mask.tag_namespaces(), "namespaces")
...
def validate_mask(new_file, key, valid_pixels=True):
    ...
    if valid_pixels:
        ...
        assert nb_pix_different/nb_pix_total < 0.4, \
